...
export function JobOrderMaster() {
  ...
  const getExportData = () => {
    const columns = [
      ...
      { header: 'Units (Order)', key: 'order_qty_unit' },
      { header: 'Production', key: 'production_qty' },
      { header: 'Units (Prod)', key: 'production_qty_unit' },
    ...
    const rows = targetOrders.map(order => {
      const rowObj: Record<string, any> = {
        ...
        itemDescription: order.item?.name || order.custom_item || '',
        order_qty: order.order_qty ?? '',
        order_qty_unit: order.order_qty_unit || '',
        production_qty: order.production_qty ?? '',
        production_qty_unit: order.production_qty_unit || '',
        status: order.status,
        start_date: order.start_date ? new Date(order.start_date).toLocaleDateString() : '',
        end_date: order.end_date ? new Date(order.end_date).toLocaleDateString() : ''
      };
  ...
  const fetchData = async () => {
    try {
      ...
      if (joFmt && joFmt.versions && joFmt.versions[0]) {
        setJoSchema(joFmt.versions[0].fields_schema || []);
      }
    } catch (err) {
      console.error(err);
    } finally {
        ...
        <div className="bg-white p-6 rounded-card border border-border shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-text-secondary">Order Number</label>
                <input 
                  type="text" 
                  disabled 
              ...
              {/* Item Description Searchable Selector / Ignore Checkbox */}
              <div onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-1">
                  <div className="flex items-center gap-2">
                    <label className="block text-sm font-medium text-text-secondary">Item Description</label>
                    {!ignoreItemMaster && (
                      <button
                        type="button"
              ...
              <div>
                <label className="block text-sm font-medium text-text-secondary">End Date / Target Date</label>
              ...
              <div>
                <label className="block text-sm font-medium text-text-secondary">Order Qty</label>
                <input
                  type="number"
                  step="any"
                  disabled={isOperations}
              ...
              <div>
                <label className="block text-sm font-medium text-text-secondary">Order Units</label>
                <input
                  type="text"
                  disabled={isOperations}
              ...
              <div>
                <label className="block text-sm font-medium text-text-secondary">Production Qty</label>
                <input
                  type="number"
                  step="any"
                  placeholder="e.g. 450"
              ...
              <div>
                <label className="block text-sm font-medium text-text-secondary">Production Units</label>
                <input
                  type="text"
                  disabled={isOperations}
                  placeholder="e.g. pcs, kg, mtr"
            ...
            {joSchema.length > 0 && (
              <div className="pt-4 border-t border-border/60">
                <h4 className="text-sm font-semibold text-text-primary mb-3">Custom Details</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {joSchema.map((field, idx) => {
                    const isDisabled = isOperations && !field.open;
                    return (
                      <div key={idx}>
                        ...
                        {field.type === 'dropdown' ? (
                          ...
                          <input
                            ...
                            disabled={isDisabled}
                            className="block w-full border border-border rounded-md px-3 py-2 text-sm bg-white disabled:bg-gray-100 disabled:text-text-secondary"
          ...
          <table className="min-w-full divide-y divide-border">
            <thead className="bg-surface">
              <tr>
                ...
                {(isAdmin || isOperations) && <th className="px-6 py-3 text-right text-xs font-medium text-text-secondary uppercase">Actions</th>}
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-border">
              {filteredOrders.map((order) => (
                <tr key={order.id} className="hover:bg-surface">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-text-primary tabular-nums">
                    {order.order_qty !== null && order.order_qty !== undefined ? order.order_qty : '—'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">
                    {order.order_qty_unit || '—'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-text-primary font-semibold tabular-nums">
                    {order.production_qty !== null && order.production_qty !== undefined ? order.production_qty : '—'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">
                    {order.production_qty_unit || '—'}
                  </td>
