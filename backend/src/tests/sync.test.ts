...
import { syncReportEntryToProduction } from '../utils/sync';
...
async function runSyncTests() {
  ...
  try {
    ...
    const entry = await tenantPrisma.$transaction(async (tx: any) => {
    ...
    const updatedEntry = await tenantPrisma.$transaction(async (tx: any) => {
