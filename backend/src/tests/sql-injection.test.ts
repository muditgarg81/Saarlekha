...
function postRequest(path: string, body: any): Promise<{ statusCode: number; data: any }> {
  ...
  for (const maliciousPayload of payloads) {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        ...
        // Query settings
        const tenantRes: any = await tx.$queryRaw`SELECT current_setting('app.current_tenant_id', true) as val`;
        const emailRes: any = await tx.$queryRaw`SELECT current_setting('app.login_email', true) as val`;
