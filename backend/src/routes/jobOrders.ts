...
jobOrdersRouter.get('/', async (req, res) => {
  ...
  try {
    ...
    if (user.role === 'OPERATIONS') {
      ...
      const allowedDeptIds = userDepts.map((d: any) => d.department_id);
...
jobOrdersRouter.post('/', requireRole(['SUPER_ADMIN', 'COMPANY_ADMIN']), async (req, res) => {
  const tenantId = req.tenantId;
  const data = req.body;
  const prismaTenant = getTenantPrisma(tenantId!);
...
jobOrdersRouter.put('/:id', requireRole(['SUPER_ADMIN', 'COMPANY_ADMIN', 'OPERATIONS']), async (req, res) => {
  const tenantId = req.tenantId;
  ...
  try {
    ...
    if (!prevOrder) return res.status(404).json({ error: 'Job order not found' });
    ...
    if (req.user!.role === 'OPERATIONS') {
      ...
      if (data.custom_data) {
        const prevCustom = (prevOrder.custom_data as Record<string, any>) || {};
...
jobOrdersRouter.delete('/:id', requireRole(['SUPER_ADMIN', 'COMPANY_ADMIN']), async (req, res) => {
  const tenantId = req.tenantId;
  const id = req.params.id as string;
  const prismaTenant = getTenantPrisma(tenantId!);
...
jobOrdersRouter.post('/bulk-delete', requireRole(['SUPER_ADMIN', 'COMPANY_ADMIN']), async (req, res) => {
  const tenantId = req.tenantId;
  const { ids } = req.body;
  const prismaTenant = getTenantPrisma(tenantId!);
...
jobOrdersRouter.get('/by-number/:orderNumber/summary', async (req, res) => {
  const tenantId = req.tenantId;
