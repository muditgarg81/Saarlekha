...
import { prisma, getTenantPrisma } from '../db/prisma';
    ...
    if (existingUser) {
      ...
      if (existingUser.google_id && !existingUser.password_hash) {
        ...
        const token = jwt.sign(
          { id: updatedUser.id, role: updatedUser.role, companyId: updatedUser.company_id },
          JWT_SECRET,
          { expiresIn: '7d' }
        );
...
authRouter.post('/google', async (req, res) => {
  ...
  try {
    ...
    res.json({ token, user: { id: user.id, email: user.email, role: user.role, companyId: user.company_id, companyName: (user as any).company?.name } });
  } catch (error: any) {
...
authRouter.post('/invite', authenticate, requireRole(['SUPER_ADMIN', 'COMPANY_ADMIN']), async (req, res) => {
  ...
  try {
    const prismaTenant = getTenantPrisma(companyId, req.user?.role);
