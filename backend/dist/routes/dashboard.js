...
exports.dashboardRouter.get('/summary', async (req, res) => {
    ...
    try {
        ...
        const manpowerCount = await prismaTenant.manpower.count();
        // Open Job Orders
        const openJobOrders = await prismaTenant.jobOrder.count({
            where: { status: 'OPEN' }
        });
        ...
        if (deptFilter) {
            ...
            const manpowerInDepts = await prismaTenant.manpower.findMany({
                where: { department_id: { in: deptFilter } },
                select: { id: true }
            });
            productionWhere.operator_id = { in: manpowerInDepts.map((m) => m.id) };
        }
        const productionRecords = await prismaTenant.productionRecord.findMany({
        ...
        const maintenanceEntries = await prismaTenant.reportEntry.findMany({
            where: {
                company_id: tenantId,
                format_version: {
                    format: {
        ...
        // Recent report entries
        const recentEntries = await prismaTenant.reportEntry.findMany({
            where: reportEntryWhere,
            take: 10,
